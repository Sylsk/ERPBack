  ...
  create: async (data) => {
    ...
    try {
      ...
      const compraResult = await client.query(
        `INSERT INTO "Compras".compras_oc 
         (id_proveedor, id_empleado, estado, subtotal, iva, total)
  ...
  update: async (id, data) => {
    ...
    const result = await pool.query(
      `UPDATE "Compras".compras_oc 
       SET ${fields.join(', ')}
       WHERE id_orden_compra = $${paramCount} RETURNING *`,
      values
    );
    return result.rows[0];
  },
  ...
  findAllWithCompleteInfo: async () => {
    try {
      ...
      result.rows.forEach(row => {
        const compraId = row.id_orden_compra;
